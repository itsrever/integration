...
// AssertRecurseLineItemRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of LineItem (e.g. [][]LineItem), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseLineItemRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aLineItem, ok := obj.(LineItem)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertLineItemRequired(aLineItem)
	})
}
