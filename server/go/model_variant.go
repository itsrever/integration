...
func AssertVariantRequired(obj Variant) error {
	...
	for _, el := range obj.Options {
		if err := AssertOptionRequired(el); err != nil {
			return err
		}
	}
	if err := AssertVariantUnitPriceRequired(obj.UnitPrice); err != nil {
		return err
	}
	return nil
}

// AssertRecurseVariantRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of Variant (e.g. [][]Variant), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseVariantRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aVariant, ok := obj.(Variant)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertVariantRequired(aVariant)
	})
}
