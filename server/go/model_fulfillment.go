...
// AssertRecurseFulfillmentRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of Fulfillment (e.g. [][]Fulfillment), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseFulfillmentRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aFulfillment, ok := obj.(Fulfillment)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertFulfillmentRequired(aFulfillment)
	})
}
